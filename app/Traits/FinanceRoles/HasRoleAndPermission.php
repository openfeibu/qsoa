...
trait HasRoleAndPermission
{
    ...
    {
        return $this->belongsToMany(config('model.finance_roles.finance_role.model.model'),'finance_user_finance_role')->withTimestamps();
    }
    ...
    {
        $permissionModel = app(config('model.finance_roles.finance_permission.model.model'));

        if (!$permissionModel instanceof Model) {
            throw new InvalidArgumentException('[model.finance_roles.finance_permission.model.model] must be an instance of \Illuminate\Database\Eloquent\Model');
        }

        return $permissionModel::select(['finance_permissions.slug', 'finance_permissions.name'])
            ->join('finance_permission_finance_role', 'finance_permission_finance_role.finance_permission_id', '=', 'finance_permissions.id')
            ->join('finance_roles', 'finance_roles.id', '=', 'finance_permission_finance_role.finance_role_id')
            ->whereIn('finance_roles.id', $this->getRoles()->pluck('id')->toArray())
            ->orWhere('finance_roles.level', '<', $this->level());
            //->groupBy(['finance_permissions.slug']);
    }
    ...
    public function menus($parent_id = 0)
    {
        ...
        return $permissionModel::select(['finance_permissions.id','finance_permissions.slug', 'finance_permissions.name','finance_permissions.icon'])
            ->join('finance_permission_finance_role', 'finance_permission_finance_role.finance_permission_id', '=', 'finance_permissions.id')
            ->join('finance_roles', 'finance_roles.id', '=', 'finance_permission_finance_role.finance_role_id')
            ->where('is_menu', 1)
