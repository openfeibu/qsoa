...
namespace App\Traits\SupplierRoles;
...
trait HasRoleAndPermission
{
    ...
    {
        return $this->belongsToMany(config('model.supplier_roles.supplier_role.model.model'),'supplier_user_supplier_role')->withTimestamps();
    }
    ...
    {
        $permissionModel = app(config('model.supplier_roles.supplier_permission.model.model'));

        if (!$permissionModel instanceof Model) {
            throw new InvalidArgumentException('[model.supplier_roles.supplier_permission.model.model] must be an instance of \Illuminate\Database\Eloquent\Model');
        }

        return $permissionModel::select(['supplier_permissions.slug', 'supplier_permissions.name'])
            ->join('supplier_permission_supplier_role', 'supplier_permission_supplier_role.supplier_permission_id', '=', 'supplier_permissions.id')
            ->join('supplier_roles', 'supplier_roles.id', '=', 'supplier_permission_supplier_role.supplier_role_id')
            ->whereIn('supplier_roles.id', $this->getRoles()->pluck('id')->toArray())
            ->orWhere('supplier_roles.level', '<', $this->level());
            //->groupBy(['supplier_permissions.slug']);
    }
    ...
    public function menus($parent_id = 0)
    {
        ...
        return $permissionModel::select(['supplier_permissions.slug', 'supplier_permissions.name','supplier_permissions.icon'])
            ->join('supplier_permission_supplier_role', 'supplier_permission_supplier_role.supplier_permission_id', '=', 'supplier_permissions.id')
            ->join('supplier_roles', 'supplier_roles.id', '=', 'supplier_permission_supplier_role.supplier_role_id')
            ->where('is_menu', 1)
    ...
    {
        return null;
        return $this->belongsToMany(config('model.supplier_roles.supplier_permission.model.model'))->withTimestamps();
    }
