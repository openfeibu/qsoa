...
trait HasRoleAndPermission
{
    ...
    {
        return $this->belongsToMany(config('model.airline_roles.airline_role.model.model'),'airline_user_airline_role')->withTimestamps();
    }
    ...
    {
        $permissionModel = app(config('model.airline_roles.airline_permission.model.model'));

        if (!$permissionModel instanceof Model) {
            throw new InvalidArgumentException('[model.airline_roles.airline_permission.model.model] must be an instance of \Illuminate\Database\Eloquent\Model');
        }

        return $permissionModel::select(['airline_permissions.slug', 'airline_permissions.name'])
            ->join('airline_permission_airline_role', 'airline_permission_airline_role.airline_permission_id', '=', 'airline_permissions.id')
            ->join('airline_roles', 'airline_roles.id', '=', 'airline_permission_airline_role.airline_role_id')
            ->whereIn('airline_roles.id', $this->getRoles()->pluck('id')->toArray())
            ->orWhere('airline_roles.level', '<', $this->level());
            //->groupBy(['airline_permissions.slug']);
    }
    ...
    public function menus($parent_id = 0)
    {
        ...
        return $permissionModel::select(['airline_permissions.slug', 'airline_permissions.name','airline_permissions.icon'])
            ->join('airline_permission_airline_role', 'airline_permission_airline_role.airline_permission_id', '=', 'airline_permissions.id')
            ->join('airline_roles', 'airline_roles.id', '=', 'airline_permission_airline_role.airline_role_id')
            ->where('is_menu', 1)
    ...
    {
        return null;
        return $this->belongsToMany(config('model.airline_roles.airline_permission.model.model'))->withTimestamps();
    }
