        ...
        switch ($exception) {
            case ($exception instanceof \App\Exceptions\RequestSuccessException):
                $responseJson = [
                    'code' => 0,
                    'status' => 'success',
                    'message' =>  $exception->getMessage() ? $exception->getMessage() : trans('error.200')
                ];
                break;
            ...
            case ($exception instanceof \App\Exceptions\DataNotFoundException):
                ...
                break;
            case ($exception instanceof \League\Flysystem\FileNotFoundException):
                $responseJson = [
                    'code' => 404,
                    'status' => 'error',
                    'message' => $exception->getMessage() ? $exception->getMessage() : trans('error.404'),
