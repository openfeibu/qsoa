    ...
    <div class="main_full">
        ...
        <div class="layui-col-md12">
            <div class="fb-main-table">
                <form class="layui-form" action="{{guard_url('quotation/'.$quotation->id)}}" method="post" lay-filter="fb-form">
                    <div class="layui-form-item">
                        <label class="layui-form-label">{{ trans('quotation.label.name') }}</label>
                        <div class="layui-input-inline">
                            <input type="text" name="name" lay-verify="required" autocomplete="off" placeholder="" class="layui-input" value="{{ $quotation->name }}">
                        </div>
                    </div>
                        ...
                        <div class="layui-input-inline">
                            <select name="airport_id"  lay-search lay-verify="required">
                                @foreach($airports as $key => $airport)
                                    <option value="{{ $airport->id }}" @if($airport->id == $quotation->airport_id) selected @endif>{{ $airport->name }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="layui-form-item">
                        ...
                        <div class="layui-input-inline">
                            <input type="text" name="billing_date" id="billing_date" lay-verify="required" autocomplete="off" placeholder="" class="layui-input" value="{{ $quotation->billing_date }}">
                        </div>
                    </div>
                    <div class="layui-form-item">
