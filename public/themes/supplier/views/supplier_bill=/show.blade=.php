                ...
                <form class="layui-form" action="{{guard_url('supplier_bill/'.$supplier_bill['id'])}}" method="post" lay-filter="fb-form">
                    <div class="layui-form-item level-high">
                        ...
                        <div class="layui-input-inline">
                            @foreach($airports as $key => $airport)
                                <input type="radio" name="airport_id" title="{{ $airport->name }}"  value="{{ $airport->id }}" lay-filter="airports" lay-verify="otherReq" @if($airport['id'] == $supplier_bill['airport_id']) checked @endif disabled>
                            @endforeach
                        </div>
                    </div>
                    <div class="layui-form-item level-high">
                        ...
                        <div class="layui-input-inline">
                            @foreach($airlines as $key => $airline)
                                <input type="radio" name="airline_id" title="{{ $airline->name }}"  value="{{ $airline->id }}" lay-filter="airlines" lay-verify="otherReq"  @if($airline['id'] == $supplier_bill['airline_id']) checked @endif disabled>
                            @endforeach
                        </div>
                    </div>
                    ...
                    @foreach($supplier_bill_info as $key => $field)
                        <div class="layui-form-item">
                            ...
                            <div class="layui-input-inline">
                                <input type="text" name="field[{{ $field['id'] }}]" autocomplete="off" placeholder="" class="layui-input" value="{{ $field['field_value'] }}">
                            </div>
                        </div>
                    @endforeach
                    <div class="layui-form-item">
